...
`naviai_container create` is an ad-hoc standalone-container helper. It does not add bind mounts or Compose labels and therefore does not place a container under the `omni_project` group. For a persistent project container, add a service to `/home/naviai/Desktop/Project/omni_project/compose.yaml`, directly reference the inspected existing image, and run it with `docker compose -p omni_project ...`.
